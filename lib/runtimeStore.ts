import { DEFAULT_MENU } from "@/lib/menuSeed"
import {
  calculateCartTotals,
  calculateItemPrice,
  calculateVat,
...
function createInitialState(): RuntimeState {
  ...
  return {
    history: {
      "menu:current": {
        id: "menu:current",
        data: { menu: cloneMenu(DEFAULT_MENU) },
        updatedAt: now,
      },
      "system:flags": {
        id: "system:flags",
...
const globalForRuntime = globalThis as unknown as {
  __NFC_RUNTIME_STATE__: RuntimeState | undefined
}

function normalizeRuntimeState(input: unknown): RuntimeState {
...
function getState() {
  if (!globalForRuntime.__NFC_RUNTIME_STATE__) {
    globalForRuntime.__NFC_RUNTIME_STATE__ = createInitialState()
  }
  trimIdempotencyRecords(globalForRuntime.__NFC_RUNTIME_STATE__)
  return globalForRuntime.__NFC_RUNTIME_STATE__
}

export function exportRuntimeStateSnapshot() {
  return JSON.parse(JSON.stringify(getState())) as RuntimeState
}

export function importRuntimeStateSnapshot(snapshot: unknown) {
  globalForRuntime.__NFC_RUNTIME_STATE__ = normalizeRuntimeState(snapshot)
}
...
export function resetMenuToDefault() {
  const state = getState()
  const menu = cloneMenu(DEFAULT_MENU)
  state.history["menu:current"] = {
    id: "menu:current",
    data: { menu },
    updatedAt: nowIso(),
...
export function submitOrder(input: {
  ...
  if (input.idempotencyKey) {
    const existing = state.idempotencyRecords[input.idempotencyKey]
    if (existing) {
  ...
  if (input.idempotencyKey) {
    state.idempotencyRecords[input.idempotencyKey] = {
...
export function importMenuCsv(csv: string) {
  ...
  return replaceMenu(Array.from(sectionsById.values()))
}
