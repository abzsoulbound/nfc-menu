...
describe("route-flow QA coverage", () => {
  ...
  it("groups /order/[id]/review into initial + add-on chronology", async () => {
    ...
    expect(review.initialOrders[0].items[0].name).toBe("Espresso")
    expect(review.addonOrders[0].items[0].name).toBe("Shakshuka")
  })
  ...
  it("rejects ordering when table is locked/closed (/t/[tagId]/closed trigger state)", () => {
